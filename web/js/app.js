...
async function loadAllData() {
  ...
  try {
    var res = await fetch(API_URL + '/init');
    ...
    document.getElementById('posts-feed').innerHTML = 
      '<div class="loading">' +
        '<p>डेटा लोड करने में समस्या</p>' +
        '<button onclick="loadAllData()" style="margin-top:10px;padding:8px 16px;cursor:pointer;">पुनः प्रयास करें</button>' +
      '</div>';
  }
}
...
async function loadDailyPost() {
  try {
    const res = await fetch(API_URL + '/posts/daily');
    if (!res.ok) return;
...
async function loadTrending() {
  try {
    const res = await fetch(API_URL + '/posts/trending?limit=5');
    if (!res.ok) return;
